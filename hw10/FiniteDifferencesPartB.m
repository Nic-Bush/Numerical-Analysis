function [ w ] = FiniteDifferencesPartB( p_q_r_equations, N, BoundryCond, endpoints,true_eq )
a = zeros(N,1);
b = zeros(N,1);
d = zeros(N,1);
c = zeros(N,1);
...
fprintf('x value \t  aproximation \t true value \t error \n')
for i= 1:N+2 
    fprintf('%8.6f \t %13.10f \t %13.10f \t %13.10f  \n',xvals(i),w(i),true_eq(xvals(i)), abs(w(i)-true_eq(xvals(i))));
end
aprox_vs_real_eq = figure;

figure(aprox_vs_real_eq)
plot(xvals,w,'--', xvals, true_eq(xvals),'o')
legend('aprox','real')
title('aprox vs real eq')
xlabel('x axis')
ylabel('y axis')
